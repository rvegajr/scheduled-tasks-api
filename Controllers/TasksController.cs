...
{
    public static HashSet<TasksController.Task> Filter(this HashSet<TasksController.Task> tasks, string filterListCSV)
    {
        var filterList = filterListCSV.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
        var filteredItems = new HashSet<TasksController.Task>();
        foreach (var svc in tasks)
        {
            foreach (var filterItem in filterList)
            {
                var regExfilterItem = new Regex("^" + Regex.Escape(filterItem).Replace("\\*", ".*").Replace("\\?", "."));
                if (regExfilterItem.Match(svc.Name).Success)
                {
